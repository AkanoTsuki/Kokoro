...
public class GameControl : MonoBehaviour
{
    ...
    public  IEnumerator AdventureFight(byte teamID)
    {
        ...
        //Debug.Log("adventureTeamList[teamID].fightRound=" + adventureTeamList[teamID].fightRound);
            ...
            while (actionMenber.Count == 0)
            {
                ...
                for (int i = 0; i < fightMenberObjects.Count; i++)
                {
                    if (fightMenberObjects[i].hpNow > 0)
                    {
                        ...
                        if (fightMenberObjects[i].actionBar >= 5000)
                        {
                            actionMenber.Add(fightMenberObjects[i]);
                            fightMenberObjects[i].actionBar = (short)(fightMenberObjects[i].actionBar - 5000);
                        }
    ...
    IEnumerator TakeDamage(byte teamID, int round, FightMenberObject actionMenber, FightMenberObject targetMenber, SkillPrototype sp, int damage, byte times, float size,bool isCri)
    {
        ...
        //表现效果
        for (byte i = 0; i < damageList.Count; i++)
        {
            AdventureMainPanel.Instance.ShowEffect(teamID, targetMenber.side, targetMenber.sideIndex, effectName, size);
            if (damage > 0)
            {
                ...
                {
                    AdventureMainPanel.Instance.ShowDamageText(teamID, targetMenber.side, targetMenber.sideIndex, "<b><color=#F86A43>-" + Random.Range(damageList[i], damageList[i] + 10) + "</color></b>", true);
        ...
        if (actionMenber!=null&& targetMenber!=null)
        {
            ...
            if (actionMenber.haloStatus)
            {
                ...
                {
                    Debug.Log("触发吸血");
                    StartCoroutine(TakeCure(teamID, round, actionMenber, null, null, Attribute.Hp, (int)(damageTotal * (DataManager.mHaloDict[heroDic[actionMenber.objectID].halo].SuckBlood / 100f)), 1));
                }
            }
            //halo反伤
